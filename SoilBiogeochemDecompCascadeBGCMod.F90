module SoilBiogeochemDecompCascadeBGCMod
  ...
  !Edit by Lei Cai@ynu--start
  use SoilBiogeochemNitrogenStateType     , only : soilbiogeochem_nitrogenstate_type
  !Edit by Lei Cai@ynu--end
  !
  implicit none
  private
  !
...
contains
  ...
  !-----------------------------------------------------------------------
!Edit by Lei Cai@ynu--start
!add soilbiogeochem_nitrogenflux_inst
  subroutine decomp_rate_constants_bgc(bounds, num_soilc, filter_soilc, &
       canopystate_inst, soilstate_inst, temperature_inst, ch4_inst, soilbiogeochem_carbonflux_inst, soilbiogeochem_nitrogenstate_inst)
!Edit by Lei Cai@ynu--end
    !
    ! !DESCRIPTION:
...
!Edit by Lei Cai@ynu--start
!add soilbiogeochem_nitrogenflux_type)
...
!Edit by Lei Cai@ynu--start	
	real(r8):: ndep_scalar(bounds%begc:bounds%endc) ! ndep scaler
!Edit by Lei Cai@ynu--start
    !-----------------------------------------------------------------------
...
!Edit by Lei Cai@ynu--start		 
         sminn_col  => soilbiogeochem_nitrogenstate_inst%sminn_col       & ! Input:  [real(r8) (:) ]  (gN/m2/s) atmospheric N deposition to soil mineral N  
!Edit by Lei Cai@ynu--end  		 
         )
...
!Edit by Lei Cai@ynu--start
	    ...
	    do j = 1, nlevdecomp
            do fc = 1,num_soilc
               c = filter_soilc(fc)
!			   ndep_scalar(c) = 1._r8
			    if (sminn_col(c)>12.8) then
					 ndep_scalar(c) =(12.8/sminn_col(c))**0.172_r8
			    else
					 ndep_scalar(c) = 1._r8
			    end if
            end do
         ...
         do j = 1,nlevdecomp
            do fc = 1,num_soilc
               ...
               decomp_k(c,j,i_litr1) = k_l1    * t_scalar(c,j) * w_scalar(c,j) * depth_scalar(c,j) * o_scalar(c,j) &
                                       * spinup_geogterm_l1(c) * ndep_scalar(c)!Edit by Lei Cai@ynu
               decomp_k(c,j,i_litr2) = k_l2_l3 * t_scalar(c,j) * w_scalar(c,j) * depth_scalar(c,j) * o_scalar(c,j) &
                                       * spinup_geogterm_l23(c) * ndep_scalar(c)!Edit by Lei Cai@ynu
               ...
               decomp_k(c,j,i_soil1) = k_s1    * t_scalar(c,j) * w_scalar(c,j) * depth_scalar(c,j) * o_scalar(c,j) &
                                       * spinup_geogterm_s1(c) * ndep_scalar(c)!Edit by Lei Cai@ynu
               decomp_k(c,j,i_soil2) = k_s2    * t_scalar(c,j) * w_scalar(c,j) * depth_scalar(c,j) * o_scalar(c,j) &
                                       * spinup_geogterm_s2(c) * ndep_scalar(c)!Edit by Lei Cai@ynu
               decomp_k(c,j,i_soil3) = k_s3    * t_scalar(c,j) * w_scalar(c,j) * depth_scalar(c,j) * o_scalar(c,j) &
                                       * spinup_geogterm_s3(c) * ndep_scalar(c)!Edit by Lei Cai@ynu
            end do
         end do
      else
         do j = 1,nlevdecomp
            do fc = 1,num_soilc
               c = filter_soilc(fc)
               decomp_k(c,j,i_litr1) = k_l1    * t_scalar(c,j) * w_scalar(c,j) * o_scalar(c,j) * spinup_geogterm_l1(c)* ndep_scalar(c)!Edit by Lei Cai@ynu
               decomp_k(c,j,i_litr2) = k_l2_l3 * t_scalar(c,j) * w_scalar(c,j) * o_scalar(c,j) * spinup_geogterm_l23(c)* ndep_scalar(c)!Edit by Lei Cai@ynu
               decomp_k(c,j,i_litr3) = k_l2_l3 * t_scalar(c,j) * w_scalar(c,j) * o_scalar(c,j) * spinup_geogterm_l23(c)* ndep_scalar(c)!Edit by Lei Cai@ynu
               decomp_k(c,j,i_soil1) = k_s1    * t_scalar(c,j) * w_scalar(c,j) * o_scalar(c,j) * spinup_geogterm_s1(c)* ndep_scalar(c)!Edit by Lei Cai@ynu
               decomp_k(c,j,i_soil2) = k_s2    * t_scalar(c,j) * w_scalar(c,j) * o_scalar(c,j) * spinup_geogterm_s2(c)* ndep_scalar(c)!Edit by Lei Cai@ynu
               decomp_k(c,j,i_soil3) = k_s3    * t_scalar(c,j) * w_scalar(c,j) * o_scalar(c,j) * spinup_geogterm_s3(c)* ndep_scalar(c)!Edit by Lei Cai@ynu
            end do
         end do
      end if
            ...
            do j = 1,nlevdecomp
               do fc = 1,num_soilc
                  ...
                  decomp_k(c,j,i_cwd)   = k_frag  * t_scalar(c,j) * w_scalar(c,j) * depth_scalar(c,j) * &
                       o_scalar(c,j) * spinup_geogterm_cwd(c)* ndep_scalar(c)!Edit by Lei Cai@ynu
               end do
            end do
         else
            do j = 1,nlevdecomp
